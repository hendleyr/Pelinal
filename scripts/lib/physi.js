window.Physijs = (function() {
	...
	Physijs.HeightfieldMesh = function ( geometry, material, mass, xdiv, ydiv) {
		...
		// --hendleyr: modified this to support transforming geometry and using y for height data;
		// think there might be an issue here with how i wrote my fragment shader/not getting appropriate vertex attributes
		// points[i] = geometry.vertices[ a + ( ( this._physijs.ypts - b - 1 ) * this._physijs.ypts ) ].y;
		
		this._physijs.type   = 'heightfield';
		this._physijs.xsize  = geometry.boundingBox.max.x - geometry.boundingBox.min.x;
		this._physijs.ysize  = geometry.boundingBox.max.z - geometry.boundingBox.min.z;
